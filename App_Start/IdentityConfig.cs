    ...
    public class EmailService : IIdentityMessageService
    {
        public async Task SendAsync(IdentityMessage message)
        {
            ...
            try
            {
                ...
                if (emailType.ToLower() == (string.Format("smtp").ToLower()))
                { 
                    //Do nothing
                }
                else
                    if (emailType.ToLower() == (string.Format("sendgrid").ToLower()))
                    {
                        //Do nothing 
                    }
                    else
                        if (emailType.ToLower() == (string.Format("test").ToLower()))
                        {
                            //Do nothing 
                        }
                        else
                                throw new Exception("smtp/SendGrind not selected!");
                ...
                {
                    case "smtp":
                        await ConfigSendSmtp(message, g);
                        break;
                    case "sendgrid":
                        await configSendGridasync(message, g);
                        break;
                    case "test":
                        TestEmail(message, g);
                        break;
                    ...
                    default:
                        throw new Exception("Something went wrong. Could not figure out if SMTP should be used or SendGrid.");
                }
            }
            catch(Exception)
            {
